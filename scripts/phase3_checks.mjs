...
async function runFunctionalFlow() {
  ...
  try {
    ...
    const vocabAnswer = page.locator('#answers button').first();
    await vocabAnswer.waitFor({ state:'visible', timeout:10_000 });
    const beforeWord = (await page.locator('#questionWord').textContent()) || '';
    await vocabAnswer.click();
    assert(!(await page.locator('#nextQuestion').isDisabled()), 'Responder vocabulario no habilitó continuar.');
    assert(!await page.locator('#vocabResult').getAttribute('hidden'), 'No se mostró el resultado de vocabulario.');
    await page.locator('#nextQuestion').click();
    await page.waitForTimeout(100);
    assert(((await page.locator('#questionWord').textContent()) || '') !== beforeWord, 'La ruta de vocabulario no avanzó al siguiente ejercicio.');

    const grammarAnswer = page.locator('#grammarAnswers button').first();
    await grammarAnswer.waitFor({ state:'visible', timeout:10_000 });
    await grammarAnswer.click();
    assert(!(await page.locator('#nextGrammar').isDisabled()), 'Responder gramática no habilitó continuar.');
    assert(!await page.locator('#grammarResult').getAttribute('hidden'), 'No se mostró el resultado de gramática.');
    console.log('✓ funcional English Fast: vocabulario y gramática responden y avanzan');
  } catch (error) {
    await saveFailure(page, 'functional-englishfast', {error:error.message});
    failures.push({label:'functional-englishfast',errors:[error.message]});
    console.error(`✗ funcional English Fast: ${error.message}`);
  } finally { await context.close(); }
}
